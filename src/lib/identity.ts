/* Identity normalization + uniqueness engine (mirrors the Go backend and
   PostgreSQL unique indexes — ONE email → ONE account, ONE phone → ONE). */
...
export const IDENTITY_MESSAGES: Record<IdentityCode, string> = {
  ...
  IDENTITY_CONFLICT: "This email and phone number belong to different Stark accounts. For your safety they can't be registered together — contact Stark Support.",
  ACCOUNT_FROZEN: "This account is frozen. Contact support to recover it.",
};

/* Trim + lowercase — Clark@Example.com ≡ clark@example.com */
...
/* Canonical Nigerian form: +234XXXXXXXXXX */
export function normalizePhone(raw: string): string | null {
  let d = raw.replace(/[^\d]/g, "");
  if (d.startsWith("00234")) d = d.slice(3);
