...
export default function Wallet() {
  ...
  const [fundEmail, setFundEmail] = useState(profile?.email ?? "");
  ...
  /* Real Paystack checkout → genuine charge → record with real reference. */
  const payWithPaystack = async () => {
    setFundBusy(true);
    setFundNote(null);
    try {
      const res = await openPaystackCheckout({ email: fundEmail.trim() || profile?.email || "customer@stark.app", amountNaira: fundAmt, name: profile?.name });
      if (res.status === "success" && res.reference) {
        await addFunds(fundAmt, res.reference);
        toast(`${money0(fundAmt)} added to your wallet`, "ok");
        setFundOpen(false);
      } else {
        setFundNote("Payment window closed before completing. Nothing was charged — try again when ready.");
      }
    } catch (e) {
      setFundNote((e as Error).message || "Could not reach Paystack. Check your connection and retry.");
    } finally {
      setFundBusy(false);
    }
  };
  ...
  const runWd = async (pin: string) => {
    ...
    try {
      const tx = await withdraw(amt, wd.bank, wd.account, wd.name || profile?.name || "");
  ...
  return (
    <div className="pb-28">
      ...
      {/* fund sheet (real Paystack) */}
      <Sheet open={fundOpen} onClose={() => !fundBusy && setFundOpen(false)} title="Add money">
        <div className="mt-3 flex flex-col">
          <div className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-2 bg-well border border-line rounded-xl px-4">
                ...
                <input className="bg-transparent outline-none py-3.5 font-display font-bold text-2xl tnum w-full" inputMode="numeric" value={fundAmt || ""} placeholder="0"
                  onChange={(e) => setFundAmt(Number(e.target.value.replace(/\D/g, "").slice(0, 7)))} />
              </div>
            </div>
            <div className="grid grid-cols-4 gap-2">
              {[1000, 2000, 5000, 10000, 20000, 50000, 100000, 250000].map((v) => (
                <button key={v} onClick={() => setFundAmt(v)} className={`press py-2.5 rounded-xl border text-xs font-bold tnum transition-colors ${fundAmt === v ? "bg-cyan text-cyanink border-cyan" : "bg-panel border-line text-sub hover:border-cyan/40"}`}>
                  {v >= 1000 ? `${v / 1000}k` : v}
                </button>
              ))}
            </div>
            <Field label="Email for receipt" type="email" placeholder="you@example.com" value={fundEmail} onChange={(e) => setFundEmail(e.target.value)} />
            {fundNote && <div className="a-rise text-[11px] font-semibold text-warn bg-warn/10 border border-warn/25 rounded-xl px-3 py-2.5">{fundNote}</div>}
            <div className="relative rounded-2xl border border-cyan/30 overflow-hidden p-5" style={{ background: "linear-gradient(150deg, #07222F 0%, #0B3247 60%, #07222F 100%)" }}>
              ...
              <div className="relative">
                ...
                <p className="font-display font-bold text-[34px] leading-none tnum tracking-tight mt-3">{money(fundAmt)}</p>
          ...
          <div className="sticky bottom-0 -mx-5 px-5 pt-3 pb-1 mt-4 bg-raised border-t border-line/50 space-y-2.5">
            <SBtn className="w-full" loading={fundBusy} disabled={fundBusy || !(fundAmt >= 100 && fundAmt <= 5000000)} onClick={payWithPaystack}>
              {fundBusy ? "Waiting for Paystack…" : <>Pay {money0(fundAmt)} with Paystack <ILock size={14} /></>}
            </SBtn>
            <p className="text-[10px] text-mute flex items-center justify-center gap-1.5"><ICard size={12} className="text-cyan shrink-0" /> You'll pay on Paystack's secure checkout. STARK never sees your card.</p>
          </div>
        </div>
      </Sheet>
